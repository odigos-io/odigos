...
func SameNodeOTLPDataCollectionEndpoint(nodeIP string) string {
...
// SameNodeOTLPHttpDataCollectionEndpoint returns the endpoint for the OTLP HTTP data collection pod on the same node.
// If the internal traffic policy is enabled, the endpoint will use the service name.
// Otherwise, it will use the node IP.
// The node IP might be passed as explicit IP or as a pattern lik "(NODE_IP)".
// Using a pattern is useful when the target node is not known once calling this function.
func SameNodeOTLPHttpDataCollectionEndpoint(nodeIP string) string {
	if feature.ServiceInternalTrafficPolicy(feature.GA) {
