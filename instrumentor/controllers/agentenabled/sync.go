...
func updateInstrumentationConfigSpec(ctx context.Context, c client.Client, pw k8sconsts.PodWorkload, ic *odigosv1.InstrumentationConfig, distroProvider *distros.Provider, effectiveConfig *common.OdigosConfiguration) (*agentInjectedStatusCondition, error) {
	cg, irls, urlTemplatizationRules, workloadObj, err := getRelevantResources(ctx, c, pw)
	if err != nil {
	...
	for containerName, containerRuntimeDetails := range runtimeDetailsByContainer {
		...
		currentContainerConfig := calculateContainerInstrumentationConfig(containerName, effectiveConfig, containerRuntimeDetails, distroPerLanguage, distroProvider.Getter, rollbackOccurred, existingBackoffReason, cg, irls, containerOverride, urlTemplatizationRules, workloadObj)
...
func calculateContainerInstrumentationConfig(containerName string,
	...
	tracesConfig, err := signalconfig.CalculateTracesConfig(tracesEnabled, effectiveConfig, containerName, filteredTemplateRules, irls, workloadObj, distro)
	if err != nil {
		return *err
	}
