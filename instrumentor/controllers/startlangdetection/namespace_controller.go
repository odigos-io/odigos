...
func (n *NamespacesReconciler) Reconcile(ctx context.Context, request ctrl.Request) (ctrl.Result, error) {
	...
	for _, dep := range deps.Items {
		if _, exists := dep.Labels[consts.OdigosInstrumentationLabel]; !exists {
			...
			_, err = reconcileWorkload(ctx, n.Client, &appsv1.Deployment{}, workload.WorkloadKindPascalCaseDeployment, req, n.Scheme)
			if err != nil {
	...
	for _, st := range sts.Items {
		if _, exists := st.Labels[consts.OdigosInstrumentationLabel]; !exists {
			...
			_, err = reconcileWorkload(ctx, n.Client, &appsv1.StatefulSet{}, workload.WorkloadKindPascalCaseStatefulSet, req, n.Scheme)
			if err != nil {
	...
	for _, ds := range dss.Items {
		if _, exists := ds.Labels[consts.OdigosInstrumentationLabel]; !exists {
			...
			_, err = reconcileWorkload(ctx, n.Client, &appsv1.DaemonSet{}, workload.WorkloadKindPascalCaseDaemonSet, req, n.Scheme)
			if err != nil {
