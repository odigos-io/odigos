...
import { isOverTime } from '@odigos/ui-components';
...
export const useNotificationStore = create<StoreState>((set, get) => ({
  ...
  addNotification: (notif) => {
    ...
    const foundThisNotif = !!get().notifications.find((n) => n.type === notif.type && n.title === notif.title && n.message === notif.message && !isOverTime(n.time, 10000)); // 10 seconds
