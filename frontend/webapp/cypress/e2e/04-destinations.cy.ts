    ...
    visitPage(ROUTES.OVERVIEW, () => {
      updateEntity(
        ...
        () => {
          ...
          cy.wait('@gql').then(() => {
            awaitToast({ withSSE: false, message: TEXTS.NOTIF_DESTINATION_UPDATED(SELECTED_ENTITIES.DESTINATION.TYPE) });
          });
        },
      );
    });
    ...
    visitPage(ROUTES.OVERVIEW, () => {
      deleteEntity(
        ...
        () => {
          ...
          cy.wait('@gql').then(() => {
            awaitToast({ withSSE: true, message: TEXTS.NOTIF_DESTINATION_DELETED(SELECTED_ENTITIES.DESTINATION.TYPE) }, () => {
              getCrdIds({ namespace, crdName, expectedError: TEXTS.NO_RESOURCES(namespace), expectedLength: 0 });
            });
          });
        },
