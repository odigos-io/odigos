...
const Container = styled.div<{ $isError: Props['isError'] }>`
  ...
  background: ${({ $isError, theme }) =>
    `linear-gradient(180deg, ${($isError ? theme.text.error : theme.colors.text) + hexPercentValues['020']} 0%, ${($isError ? theme.text.error : theme.colors.text) + hexPercentValues['005']} 100%)`};
`;
