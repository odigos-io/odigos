...
const DrawerFooter: React.FC<Props> = ({ isOpen, onSave, saveLabel = 'Save', onCancel, cancelLabel = 'Cancel', onDelete, deleteLabel = 'Delete' }) => {
  ...
  return (
    <Transition enter={isOpen}>
      <FooterButton id='drawer-save' variant='primary' onClick={onSave}>
        {saveLabel}
      </FooterButton>
      <FooterButton id='drawer-cancel' variant='secondary' onClick={onCancel}>
        {cancelLabel}
      </FooterButton>

      <AlignRight>
        <FooterButton id='drawer-delete' variant='tertiary' onClick={onDelete}>
