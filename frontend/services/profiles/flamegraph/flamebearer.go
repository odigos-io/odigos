...
import pyrominheap "github.com/grafana/pyroscope/pkg/util/minheap"

// Flamebearer and FlamebearerProfile mirror the JSON the Pyroscope UI consumes. We already use
// github.com/grafana/pyroscope for OTLP→internal profile conversion (see pyroscope_convert.go);
// Grafana does not publish stable exported Go types for this JSON or for merged-tree → flamebearer
// encoding (that logic lives in internal packages), so these structs and TreeToFlamebearer stay local.
//
...
// TreeToFlamebearer converts a merged sample Tree into the Pyroscope flamebearer JSON shape.
// It walks the tree depth-first, assigns x offsets per child, folds small branches into "other"
// using the same min-heap threshold as Grafana Pyroscope (pkg/model/tree + flamegraph), then
// delta-encodes x positions. Grafana exports this logic inside internal packages, not as a stable
// import for third parties, so we keep a local implementation aligned with theirs.
