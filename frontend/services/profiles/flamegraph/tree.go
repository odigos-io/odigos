...
// SymbolStats is one row for the symbol table (top table / hot list style).
...
// InsertStack adds one sample of weight value along a root-first stack (caller before callee).
// Skips non-positive values, empty stacks, and empty frame names; merges into existing nodes by name.
