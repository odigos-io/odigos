...
// types; we still unmarshal OTLP JSON ourselves because the UI stores JSON chunks from the collector.
//
// Pipeline in this file:
//  1. tryPyroscopeOTLP: protojson.Unmarshal each chunk → ExportProfilesServiceRequest.
...
//  4. SamplesFromOTLPChunk is the entry used by profile_builder: try Pyroscope path, else legacy JSON fallback.
package flamegraph
...
// tryPyroscopeOTLP unmarshals one OTLP Profiles JSON chunk and runs Pyroscope's ConvertOtelToGoogle.
