...
// into merged stack samples and flame-graph JSON. OTLP profiles are converted to an intermediate
// pprof-style representation via otlp.ConvertOtelToGoogle (third-party ingester), then mapped to
// our Sample type.
//
// Pipeline in this file:
//  1. decodeOTLPChunkToSamples: proto.Unmarshal each chunk → ExportProfilesServiceRequest.
//  2. Per profile: collapseMultiValueSamplesToSingleCounter, ensureServiceNameInSamples,
//     then convertOTLPProfileToSamples → pprof-style frames.
//  3. convertOTLPProfileToSamples wraps otlp.ConvertOtelToGoogle and maps LocationLines to Sample{Stack, Value}.
...
const (
	...
	failReasonProfileExtractPrefix = "otlp_convert_profile_extract_failed:"
	failReasonNoSamplesFromConvert = "convert_otel_to_google_yielded_no_samples"
)

// decodeOTLPChunkToSamples unmarshals one OTLP protobuf chunk, runs ConvertOtelToGoogle per
...
func decodeOTLPChunkToSamples(chunk []byte) (samples []Sample, ok bool, failReason string) {
	if len(chunk) == 0 {
	...
	for _, rp := range req.ResourceProfiles {
		...
		for _, sp := range rp.ScopeProfiles {
			...
			for _, p := range sp.Profiles {
				...
				samples, extracted, extractReason := convertOTLPProfileToSamples(p, req.Dictionary)
				if !extracted {
					if extractReason == "" {
...
// collapseMultiValueSamplesToSingleCounter adapts chunks where sample values carry multiple counters
// but sample type contains a single entry. The OTLP→pprof step expects lengths to match.
func collapseMultiValueSamplesToSingleCounter(profile *otelProfile.Profile) {
	if profile == nil {
		return
	}
...
// ensureServiceNameInSamples adds service.name to profile samples when missing so the ingester
// matches common OTLP expectations. defaultProfileServiceName is used only when the OTLP
...
// convertOTLPProfileToSamples runs OTLP→pprof conversion and turns the result into our Sample slice.
func convertOTLPProfileToSamples(src *otelProfile.Profile, dictionary *otelProfile.ProfilesDictionary) ([]Sample, bool, string) {
...
// extractGoogleProfile reads a pprof Profile from the ingester conversion output struct.
// Field layout may vary by dependency version; we locate by type first.
func extractGoogleProfile(cp interface{}) *googleProfile.Profile {
	...
	for i := 0; i < v.NumField(); i++ {
		...
		case googleProfilePtrType:
			...
			// Converted type may use unexported fields; unsafe dereference couples us to their
...
func googleProfileToSamples(p *googleProfile.Profile) []Sample {
	...
	for _, s := range p.Sample {
		...
		// LocationId order: outer frame first (matches stack indices in OTLP).
