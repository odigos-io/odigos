...
// Add always stores a whole new chunk or skips it (if larger than maxBytes when maxBytes > 0).
...
// Snapshot returns a shallow copy of the chunk list: new outer slice, same inner []byte backings
// as the live buffer. Callers must treat each element as read-only; they must not mutate bytes.
// Concurrent Add/trim take the write lock; concurrent Snapshot calls use the read lock.
