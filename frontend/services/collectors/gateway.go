...
func findLastRolloutTime(ctx context.Context, dep *appsv1.Deployment) string {
    if dep.Spec.Template.Annotations != nil {
        if v, ok := dep.Spec.Template.Annotations["kubectl.kubernetes.io/restartedAt"]; ok { return v }
    }
    if dep.Spec.Selector == nil { return "" }
    ...
    if err != nil || len(rsList.Items) == 0 { return "" }
    owned := make([]appsv1.ReplicaSet, 0, len(rsList.Items))
    for _, rs := range rsList.Items {
        for _, owner := range rs.OwnerReferences {
            if owner.Kind == "Deployment" && owner.UID == dep.UID { owned = append(owned, rs); break }
        }
    }
...
func computeGatewayHPA(dep *appsv1.Deployment, hpa *autoscalingv2.HorizontalPodAutoscaler) *model.HorizontalPodAutoscalerInfo {
    if hpa == nil { return nil }
    h := &model.HorizontalPodAutoscalerInfo{}
    if hpa.Spec.MinReplicas != nil { v := int(*hpa.Spec.MinReplicas); h.Min = &v }
    max := int(hpa.Spec.MaxReplicas); h.Max = &max
    cur := int(dep.Status.Replicas); h.Current = &cur
    if hpa.Status.DesiredReplicas > 0 { d := int(hpa.Status.DesiredReplicas); h.Desired = &d } else if dep.Spec.Replicas != nil { d := int(*dep.Spec.Replicas); h.Desired = &d }
    return h
}
