...
This setup of devcontainer allows you to quickly get started with Odigos and a local Kubernetes cluster using `k3d`.
...
## Full Odigos Setup (devcontainer + k3d cluster)
1. Create k3d cluster (can run locally as well)
```sh
...
odigos ui
```
## (Re)connecting to the `k3d` cluster
If you re-started the devcontainer or the k3d cluster you might need to reconnect the devcontainer to the k3d cluster. You can do so by running the following script from within the devcontainer:
```sh
./.devcontainer/connect-k3d.sh
```

## Developers Setup (or: how to plug-and-play your own stuff during development)
> Note: The cluster is using `k3d` at the host level and we are using a devcontainer, which are running on the same docker VM, therefore - any changes to the cluster (like installing `odigos` or `jaeger`) need to be applied against the docker daemon at the host; Therefore - we need to make sure that the devcontainer can communicate with the the kubeconfig(`kubectl`), k3d, docker daemon, etc. Overall, everything is set up to work out of the box, but if you need to make any special changes to the cluster or other magic - the bugs will probably reside at the above domains.

### Overall deployment flow
The flow of deploying a new version of Odigos to the `k3d` cluster is as follows:
1. Build the image(s) at your desired tag
2. Load the tag into k3d
...
Following these steps will allow you to build, load and deploy a custom version of Odigos to your `k3d` cluster.
...
```
By doing this, you will build the image and load it into k3d so everything should be set up properly.
...
#### Loading a specific component into k3d
```sh
make load-to-k3d-% TAG=<YOUR_DESIRED_TAG>
# Options:
# 1. load-to-k3d-instrumentor
# 2. load-to-k3d-autoscaler
# 3. load-to-k3d-scheduler
# 4. load-to-k3d-odiglet
# 5. load-to-k3d-collector
# 6. load-to-k3d-ui
# 7. load-to-k3d-cli
# 8. load-to-k3d-agents
```
