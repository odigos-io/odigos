		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectTLSConfig {
				...
				if tt.config["OTLP_HTTP_MTLS_ENABLED"] == "true" {
					assert.Equal(t, "${ODIGOS_DEST_test-id_OTLP_HTTP_CLIENT_CERT_PEM}", tlsConfig["cert_pem"])
					assert.Equal(t, "${ODIGOS_DEST_test-id_OTLP_HTTP_CLIENT_KEY_PEM}", tlsConfig["key_pem"])
				} else {
			...
			if tt.expectedAuth {
				...
				if tt.expectedExtName == "oauth2client/otlphttp-test-id" {
					...
					assert.Equal(t, "${ODIGOS_DEST_test-id_OTLP_HTTP_OAUTH2_CLIENT_SECRET}", extConfig["client_secret"])
