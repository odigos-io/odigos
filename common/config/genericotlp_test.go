		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectTLSConfig {
				...
				if tt.config["OTLP_GRPC_MTLS_ENABLED"] == "true" {
					assert.Equal(t, "${ODIGOS_DEST_test-id_OTLP_GRPC_CLIENT_CERT_PEM}", tlsConfig["cert_pem"])
					assert.Equal(t, "${ODIGOS_DEST_test-id_OTLP_GRPC_CLIENT_KEY_PEM}", tlsConfig["key_pem"])
				} else {
