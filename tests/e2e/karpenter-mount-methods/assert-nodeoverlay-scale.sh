...
repair_cni() {
  local server_node cni_pod
  server_node=$(kubectl get nodes -l node-role.kubernetes.io/control-plane -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || true)
  if [[ -z "${server_node}" ]]; then
    return 0
  fi
  # kind used kindnet; k3d/k3s uses flannel (often embedded). Restart any matching agent pod if present.
  for label in 'app=kindnet' 'app=flannel' 'k8s-app=kube-flannel-ds'; do
