...
- [k0s](https://docs.k0sproject.io/) in Docker — full upstream Kubernetes in privileged Docker containers (macOS + Linux/CI). Helper: `./scripts/k0s-cluster.sh`.
...
- Fresh Kubernetes cluster in kubectl context. For local development, you can use k0s-in-Docker but also managed clusters like EKS should work. you can create the cluster with `make dev-tests-k0s-cluster`.
...
- Odigos component images (instrumentor, autoscaler, odiglet etc) tagged with `e2e-test` preloaded to the cluster. If you are using k0s you can run: `TAG=e2e-test make build-images load-to-k0s`.
