...
func mapUrlTemplatizationProcessorToActionRequests(ctx context.Context, _ client.Client, obj client.Object) []reconcile.Request {
	if obj.GetName() != consts.URLTemplatizationProcessorName {
		return nil
	}
	return []reconcile.Request{{
		NamespacedName: types.NamespacedName{
			Namespace: obj.GetNamespace(),
			Name:      k8sconsts.URLTemplatizationNamespaceSyncKey,
		},
	}}
}
			...
			builder.WithPredicates(
				predicate.And(
					...
					predicate.NewPredicateFuncs(func(object client.Object) bool {
						return object.GetName() == consts.URLTemplatizationProcessorName
					}),
				),
			)).
