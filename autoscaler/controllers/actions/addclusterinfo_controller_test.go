	...
	Context("When creating legacy AddClusterInfo Actions", func() {
		...
		It("Should update existing migrated Action when legacy action changes", func() {
			...
			Eventually(func() bool {
				...
				return migratedAction.Spec.Notes == "Updated notes" &&
					len(migratedAction.Spec.AddClusterInfo.ClusterAttributes) == 2 &&
					migratedAction.Spec.AddClusterInfo.OverwriteExistingValues == true
			}, timeout, interval).Should(BeTrue())
		})
	})
})
