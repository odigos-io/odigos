...
// exercise addAbnormalGatewayExporter against this fixture instead of
...
func TestAddAbnormalGatewayExporter_Disabled(t *testing.T) {
	t.Run("nil_config_noop", func(t *testing.T) {
		c := configWithTracesIn()
		require.NoError(t, addAbnormalGatewayExporter(c, "odigos-system", nil))

		_, hasExp := c.Exporters[commonconf.AbnormalGatewayExporter]
	...
	t.Run("explicit_false_noop", func(t *testing.T) {
		off := false
		c := configWithTracesIn()
		require.NoError(t, addAbnormalGatewayExporter(c, "odigos-system", &common.AbnormalConfiguration{Enabled: &off}))
...
func TestAddAbnormalGatewayExporter_NoTracesInPipelineNoop(t *testing.T) {
	on := true
	c := &config.Config{Service: config.Service{Pipelines: map[string]config.Pipeline{}}}
	require.NoError(t, addAbnormalGatewayExporter(c, "odigos-system", &common.AbnormalConfiguration{Enabled: &on}))

	_, hasExp := c.Exporters[commonconf.AbnormalGatewayExporter]
	assert.False(t, hasExp, "exporter must not be registered when there is no root traces pipeline to tap")
}

func TestAddAbnormalGatewayExporter_EnabledAppendsExporterToRootPipeline(t *testing.T) {
	on := true
	c := configWithTracesIn()
	require.NoError(t, addAbnormalGatewayExporter(c, "odigos-system", &common.AbnormalConfiguration{Enabled: &on}))

	exp, ok := c.Exporters[commonconf.AbnormalGatewayExporter].(config.GenericMap)
	require.True(t, ok, "exporter must be registered")
	assert.Equal(t, k8sconsts.AbnormalOtlpGrpcEndpoint("odigos-system"), exp["endpoint"])
	...
	assert.Equal(t, []string{"odigosrouterconnector/traces", commonconf.AbnormalGatewayExporter}, rootPipe.Exporters,
		"side-channel exporter must be appended alongside the existing destination router exporter")
}

func TestAddAbnormalGatewayExporter_DoesNotCreateExtraPipelineOrConnector(t *testing.T) {
...
func TestAddAbnormalGatewayExporter_PreservesExistingDestinationConfig(t *testing.T) {
	on := true
	c := configWithTracesIn()
	...
	_, abn := c.Exporters[commonconf.AbnormalGatewayExporter]
	assert.True(t, dest, "destination exporter must not be removed")
	assert.True(t, abn, "side-channel exporter must be added")
