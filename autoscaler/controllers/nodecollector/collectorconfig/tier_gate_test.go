...
func TestMetricsConfig_EbpfReceiverPipelineGatedByTier(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if !tc.wantWired && !ok {
				// With no other metrics source enabled, community tier has no receivers left and
				// the pipeline is skipped entirely rather than emitted empty. Also acceptable.
				return
			}
