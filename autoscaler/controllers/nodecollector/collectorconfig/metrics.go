...
func metricsReceivers(metricsConfigSettings *odigosv1.CollectorsGroupMetricsCollectionSettings, tier common.OdigosTier) (config.GenericMap, []string) {
	...
	// odigosebpfreceiver only exists in the enterprise collector image - the OSS collector
	// doesn't have it compiled in, so wiring it into community tier's config would make every
	// OSS node collector fail to start (unknown component type).
