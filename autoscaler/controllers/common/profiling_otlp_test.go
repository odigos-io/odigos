...
func TestOdigosSymbolizeProcessorConfig(t *testing.T) {
	...
	// Configured → passed through as bytes; the processor derives both its
	// internal guards from this one value (verified in the symbolize package).
	n := 200
	got := OdigosSymbolizeProcessorConfig(&odigoscommon.ProfilingSymbolizationConfiguration{MaxMemoryMiB: &n})
	assert.Equal(t, int64(200)<<20, got["max_memory_bytes"])
}
