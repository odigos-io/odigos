...
const (
	odigosVersionConfigMap = "odigos-version"
	odigosVersionKey       = "ODIGOS_VERSION"
...
func (r *OdigosReconciler) install(ctx context.Context, odigos *operatorv1alpha1.Odigos) (ctrl.Result, error) {
	...
	for _, o := range odigosList.Items {
		if o.GetName() != odigos.GetName() {
			...
			return ctrl.Result{Requeue: false}, utilerrors.NewAggregate(
				[]error{errors.New("odigos is already installed in namespace " + ns),
					r.Status().Update(ctx, odigos),
				})
		}
	}

	version := os.Getenv(odigosVersionKey)
	if len(version) > 0 {
		...
		return ctrl.Result{Requeue: false}, utilerrors.NewAggregate(
			[]error{errors.New("could not determine Odigos version from odigos-version configmap or ODIGOS_VERSION environment variable"),
				r.Status().Update(ctx, odigos),
			})
	}
	...
	if k8sVersion != nil {
		if k8sVersion.LessThan(k8sconsts.MinK8SVersionForInstallation) {
			...
			return ctrl.Result{Requeue: false}, utilerrors.NewAggregate(
				[]error{errors.New("odigos requires Kubernetes version " + k8sconsts.MinK8SVersionForInstallation.String() + " but found " + k8sVersion.String()),
					r.Status().Update(ctx, odigos),
				})
		}
	...
	if odigos.Spec.APIKey != "" {
		...
		if err != nil {
			...
			return ctrl.Result{Requeue: false}, utilerrors.NewAggregate(
				[]error{errors.New("invalid Odigos API Key format"),
					r.Status().Update(ctx, odigos),
				})
		}
	...
	resourceManagers := resources.CreateResourceManagers(r.KubeClient, ns, odigosTier, &odigosProToken, &odigosConfig, version)
