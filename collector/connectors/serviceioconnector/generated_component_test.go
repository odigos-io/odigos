...
var typ = component.MustNewType("serviceio")
...
func TestComponentLifecycle(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name+"-shutdown", func(t *testing.T) {
			c, err := tt.createFn(context.Background(), newTestConnectorSettings(typ), cfg)
			require.NoError(t, err)
			err = c.Shutdown(context.Background())
			require.NoError(t, err)
		})
		t.Run(tt.name+"-lifecycle", func(t *testing.T) {
			firstConnector, err := tt.createFn(context.Background(), newTestConnectorSettings(typ), cfg)
			require.NoError(t, err)
			...
			secondConnector, err := tt.createFn(context.Background(), newTestConnectorSettings(typ), cfg)
			require.NoError(t, err)
