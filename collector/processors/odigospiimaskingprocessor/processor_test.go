...
func TestBuildFormatMaskingRegex(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "json quoted value with spaces",
			key:    "password",
			format: actions.FormatJSON,
			input:  `{"password": "my secret pass", "name": "foo"}`,
			want:   `{"password": "****", "name": "foo"}`,
		},
		{
			name:   "json quoted value with escaped quotes",
			key:    "password",
			format: actions.FormatJSON,
			input:  `{"password": "my \"secret\" pass", "name": "foo"}`,
			want:   `{"password": "****", "name": "foo"}`,
		...
		{
			name:   "sql quoted value with spaces",
			key:    "password",
			format: actions.FormatSQL,
		...
		{
			name:   "sql quoted value with escaped quotes",
			key:    "password",
			format: actions.FormatSQL,
			input:  `WHERE password = 'my ''secret'' pass' AND status = 'ok'`,
			want:   `WHERE password = '****' AND status = 'ok'`,
