...
Every MCP tool call is audit-logged by the Odigos UI, so you can see exactly what
an agent did. Each entry records the tool name, the outcome, and the duration.

For example:

- `create_destination` — a write that added a backend, logged with the
  initiating agent, success or failure, and how long it took.
- `enable_source_profiling` — a write attempted while in read-only mode, recorded
  as a rejected call.
