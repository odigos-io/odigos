...
- Odigos Enterprise **v1.30.0 or later**, installed with a valid on-prem token
...
Set `ui.mcp.enabled: false` to disable the `/mcp` endpoint entirely. For the full
list of Helm values, see [Installation Options](/enterprise/setup/installation-options).
...
Every write tool accepts a `dry_run` argument (default `true`) that the agent
sets in the tool call. This follows the standard
[dry-run convention](https://kubernetes.io/docs/reference/using-api/api-concepts/#dry-run)
used across Kubernetes tooling (for example, `kubectl --dry-run`); Odigos exposes
it as a tool argument. With `dry_run: true` the tool validates and previews the
change without applying it; set `dry_run: false` to apply it. For example, a
write call includes it alongside the tool's other arguments:
...
When the server is in read-only mode, write tools are not executed even with
`dry_run: false`.
