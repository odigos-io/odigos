...
Those flags only provide the pull secret in the Odigos namespace. With `k8s-init-container` on Kubernetes 1.35+, instrumented pods also need that secret in their own namespace. Set `syncOdigosPullSecrets: true` to let Odigos copy it, or sync `odigos-enterprise-registry` yourself. See [Init container image pull secrets](/enterprise/instrumentations/configuration/mount-method#init-container-image-pull-secrets).
