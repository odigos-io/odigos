...
| Component Name | Memory Request | Memory Limit | CPU Request | CPU Limit |
|---------------|---------------|-------------|------------|----------|
| odigos-gateway      | 600Mi         | 600Mi       | 1m       | 1m     |

Read more [here](/oss/pipeline/configuration).

### Per Node

3 DaemonSets are running on every node in the cluster, consuming resources as outlined below:
...
You can override these values by providing a custom configuration (read more [here](/oss/pipeline/configuration)).
