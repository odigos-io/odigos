...
- **CLICKHOUSE_INSECURE_SKIP_VERIFY** `boolean` : Insecure Skip Verify. Skip TLS certificate verification
  - This field is optional and defaults to `False`
- **CLICKHOUSE_USE_CUSTOM_CA** `boolean` : Use Custom CA Certificate. Use a custom CA certificate instead of the system root CA
  - This field is optional and defaults to `False`
- **CLICKHOUSE_CA_PEM** `string` : Certificate Authority (CA PEM). CA certificate in PEM format to verify the server
  - This field is optional
