...
```
  ...
  -n, --namespace string                 namespace to uninstall Odigos from (overrides auto-detection)
      ...
      --recover-terminating-source-crd   strip Source finalizers and wait for a Terminating sources.odigos.io CRD to finish deleting (usually after cleanup Job timeout)
