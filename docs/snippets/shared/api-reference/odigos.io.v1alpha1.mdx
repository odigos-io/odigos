...
<td>
   <p>Controls how span metrics interact with head-sampling decisions.
Possible values:</p>
<ul>
<li>&quot;sampled-spans-only&quot; (default): metrics are computed only from sampled spans;
dropped spans are discarded at the agent level.</li>
<li>&quot;record&quot;: all spans are forwarded to the collector for metric computation,
then dropped spans are deleted in the pipeline (higher resource usage).</li>
</ul>
</td>
</tr>
<tr>
