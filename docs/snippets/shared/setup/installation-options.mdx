...
On GKE, Google may warn that a custom admission controller can affect `kube-system`. The Odigos **pod mutating webhook** is cluster-wide so it can inject agents into application namespaces. It uses `failurePolicy: Ignore`, so webhook errors do not block pod creates in `kube-system`. Ignored namespaces (including `kube-system` by default) are not shown for instrumentation and are not instrumented unless you create a Source for them.
...
<Tabs>
  ...
  <Tab title="Operator">
       ...
       Leave the **On Prem Token** field blank to install Odigos Open Source. For Enterprise, use the [Enterprise installation guide](https://enterprise.docs.odigos.io/enterprise/setup/installation-options).
