...
Sampling in Odigos is **per trace**. The decision to keep or drop is made **at the root span** and applies to every span in the trace. Partial retention would break parent–child links, timing, and attributes that span the whole request, so a kept trace is always **complete** from the root span through its children.
