...
#### 1. Install Homebrew
    ...
    Homebrew is a package manager for macOS that simplifies the installation of software packages. We will use Homebrew to install Docker Desktop, k3d (local Kubernetes), and the Odigos CLI.
...
If you see a version number, Homebrew has been successfully installed.
    ...
    <Warning>
      **Mac users**: please avoid using Docker Desktop built-in Kubernetes cluster as it [does not](https://docs.docker.com/storage/bind-mounts/#configure-bind-propagation) support bind propagation. Use k3d on top of Docker instead — eBPF instrumentation runs inside the Linux node containers.
    </Warning>

    k3d runs a lightweight Kubernetes (k3s) cluster in Docker and is used for local Odigos development and demos.
...
This is the end of the one-time installations and the following steps will be required every time:
    ...
    Now that you have k3d installed, let's create a Kubernetes cluster:

    1. In the Terminal, copy and paste the following command and press `Enter`:

    ```bash
    k3d cluster delete odigos-demo
    ```
    ...
    ```bash
    k3d cluster create odigos-demo
    ```

    This command creates a new Kubernetes cluster named "odigos-demo". You'll see a lot of text appear in the Terminal. This is normal. Wait until you see a message that the cluster is ready and the command prompt appears again.

    3. Verify that the cluster is running:

    ```bash
    kubectl cluster-info --context k3d-odigos-demo
    ```
