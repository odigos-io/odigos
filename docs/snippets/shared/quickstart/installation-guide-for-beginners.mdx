...
#### 1. Install Homebrew
    ...
    Homebrew is a package manager for macOS that simplifies the installation of software packages. We will use Homebrew to install Docker Desktop, k0s (local Kubernetes), and the Odigos CLI.
...
If you see a version number, Homebrew has been successfully installed.
    ...
    #### 3. Prepare for k0s (local Kubernetes)

    <Warning>
      **Mac users**: please avoid using Docker Desktop built-in Kubernetes cluster as it [does not](https://docs.docker.com/storage/bind-mounts/#configure-bind-propagation) support bind propagation. Use k0s-in-Docker instead — eBPF runs inside the Linux node containers.
    </Warning>

    k0s runs a full upstream Kubernetes cluster in Docker and is used for local Odigos development and demos. You only need Docker; the Odigos repository includes `./scripts/k0s-cluster.sh` to create the cluster.

This is the end of the one-time installations and the following steps will be required every time:

    ### Setting up the cluster
    Now that Docker is ready, let's create a Kubernetes cluster. Open a Terminal in the Odigos repository directory:

    1. Remove any previous local k0s cluster (safe if none exists):
    ...
    Wait until you see a message that the k0s cluster is ready and the command prompt appears again.
    ...
    You should see two Ready nodes: `k0s-controller` and `k0s-worker1`.
