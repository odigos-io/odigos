...
**AWS Credentials**<br />
Go to the **[📖 docs](https://docs.aws.amazon.com/sdk-for-go/v1/developer-guide/configuring-sdk.html#specifying-credentials) > Specifying Credentials** and follow the instructions.

<Warning>
  For ingesting logs via OpenTelemtry, ensure that you have the necessary AWS Credentials and permissions set up in your cluster.

  If you do not specify the AWS Credentials, then you will have to define all `optional` fields as if they were `required`.
</Warning>
...
- **AWS_CLOUDWATCH_ENDPOINT** `string` : Endpoint. The CloudWatch Logs service endpoint which the requests are forwarded to.
  - This field is optional
- **AWS_CLOUDWATCH_LOG_RETENTION** `string` : Log Retention. LogRetention is the option to set the log retention policy for only newly created CloudWatch Log Groups. The value is in days, 0 = Never Expire.
...
- **AWS_CLOUDWATCH_RAW_LOG** `boolean` : Raw Log. If set to true, only the log message will be exported to CloudWatch Logs. This needs to be set to true for EMF logs.
  - This field is optional
- **AWS_CLOUDWATCH_SENDING_QUEUE_ENABLED** `boolean` : Enable Sending Queue.
  - This field is optional and defaults to `True`
- **AWS_CLOUDWATCH_SENDING_QUEUE_NUM_CONSUMERS** `string` : Number of Consumers. Number of consumers that will consume from the sending queue. This parameter controls how many consumers will consume from the sending queue in parallel; ignored if `enabled` is `false`.
  - This field is optional
- **AWS_CLOUDWATCH_SENDING_QUEUE_SIZE** `string` : Queue Size. Maximum number of batches kept in memory before dropping; ignored if `enabled` is `false`.
  - This field is optional
  ...
  <Step>
    ...
    spec:
      data:
        ...
        # AWS_CLOUDWATCH_SENDING_QUEUE_ENABLED: <Enable Sending Queue (default: True)>
        # AWS_CLOUDWATCH_SENDING_QUEUE_NUM_CONSUMERS: <Number of Consumers>
        # AWS_CLOUDWATCH_SENDING_QUEUE_SIZE: <Queue Size>
