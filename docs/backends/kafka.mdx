...
<img src='https://d15jtxgb40qetw.cloudfront.net/kafka.svg' alt='kafka' width="100" height="100" className="not-prose" />
...
- **KAFKA_SENDING_QUEUE_ENABLED** `boolean` : Enable Sending Queue.
  - This field is optional and defaults to `True`
- **KAFKA_SENDING_QUEUE_NUM_CONSUMERS** `string` : Number of Consumers. Number of consumers that dequeue batches; ignored if `enabled` is `false`.
  - This field is optional and defaults to `10`
- **KAFKA_SENDING_QUEUE_SIZE** `string` : Queue Size. Maximum number of batches kept in memory before dropping data; ignored if `enabled` is `false`. User should calculate this as `num_seconds` * `requests_per_second` where: `num_seconds` is the number of seconds to buffer in case of a backend outage, `requests_per_second` is the average number of requests per seconds.
  ...
  <Step>
    ...
    spec:
      data:
        ...
        # KAFKA_SENDING_QUEUE_ENABLED: <Enable Sending Queue (default: True)>
        # KAFKA_SENDING_QUEUE_NUM_CONSUMERS: <Number of Consumers (default: 10)>
        # KAFKA_SENDING_QUEUE_SIZE: <Queue Size (default: 1000)>
