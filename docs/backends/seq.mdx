...
- **SEQ_ENDPOINT** `string` : OTLP HTTP Endpoint. The format is `host:port`, `host` is required and equals the cluster internal DNS record of the Seq instance, `port` is required and equals any of the exposed ports of the Seq instance, do not add the rest of the path `/ingest/otlp/v1/*`.
