...
## Configuring Groundcover inCloud Managed Backend

Groundcover [inCloud Managed](https://docs.groundcover.com/architecture/incloud-managed) is an enterprise solution designed for installing groundcover’s observability backend infrastructure within your own cloud environment.

- **GROUNDCOVER_ENDPOINT** - Endpoint, the format is `host:port`.
  - `host` is required, your `inCloud_Site` is part of the configuration provided to you by Groundcover when setting up the managed inCloud backend.
