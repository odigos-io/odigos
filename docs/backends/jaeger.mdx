...
- **JAEGER_CA_PEM** - Certificate Authority in PEM format. This is required only if Jaeger requires client authentication (TLS).
...
spec:
  data:
    ...
    # JAEGER_TLS: <Enable TLS (secure connection)>
