...
### Okta example (values reference)

If you’re using Okta as your OIDC provider, these are the most common values you’ll need:

- **`ui.oidcTenantUrl`**: your Okta issuer URL (commonly `https://<your-okta-domain>/oauth2/default`)
- **`ui.oidcClientId`**: the “Client ID” from the Okta app integration
- **`ui.oidcClientSecret`**: the “Client secret” from the Okta app integration
