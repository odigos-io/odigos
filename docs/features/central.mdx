...
- **Centralized Access Control** - Manage user authentication and authorization through Keycloak with support for SSO providers like Okta
...
| **Redis**           | In-memory data store for configuration state                                                                    |
| **Keycloak**        | Identity provider for authentication and SSO integration                                                        |
...
- **kubectl** configured with cluster-admin privileges (or equivalent RBAC)
...
<Tabs>
  <Tab title="CLI">
    ...
    ```bash
    odigos pro central install --onprem-token <token>
    ```
    ...
    |------|---------|-------------|
    | `--onprem-token` | (required) | Your Odigos Enterprise license token |
    | `--central-admin-password` | (auto-generated) | Admin password for Keycloak. If not provided, a random password is generated |
    | `--central-admin-user` | `admin` | Admin username for Keycloak |
    | `--namespace`, `-n` | `odigos-central` | Target namespace for installation |
    | `--version` | (current) | Specify a specific version to install |
    | `--central-storage-class-name` | (cluster default) | StorageClassName for Keycloak PVC. Omit to use cluster default |
    | `--central-max-message-size` | (default) | Maximum gRPC message size in bytes |
    | `--image-pull-secrets` | (none) | Comma-separated list of secret names for private registries |

    **Example with all options:**
    ```bash
    odigos pro central install \
      --onprem-token <token> \
      --central-admin-user admin \
      --central-admin-password MySecurePass123 \
      --namespace odigos-central \
      --version v1.0.0 \
      --central-storage-class-name gp3 \
