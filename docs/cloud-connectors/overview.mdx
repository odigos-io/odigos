...
Every workload type supports two ways of getting instrumented, chosen per type when you add or edit a connector:
...
There is no separate "mode" toggle — it's just whether you turned on the instrumentation capability for that type. This is what makes self-managed workloads safe by default: until you grant instrumentation access, Odigos cannot touch the resource no matter what else is enabled.
