...
You create the construct once, then call `instrument()` for each container you want instrumented. Unlike
Lambda, there's no aspect that sweeps a whole stack — you name each container.

## How it works

Fargate has no equivalent of a Lambda layer, so the agent files have to be put into the task at startup.
The construct sets up four things that only work together:
...
**Straight from your application to your destination.** This is the substantive difference from Lambda,
where telemetry goes to a local collector first and gets forwarded after the response is returned. A
Fargate task has no such collector, so a slow or unreachable destination is felt by your application's own
exporter.

Practically, that means the destination endpoint has to be reachable from the task's network.
...
| Language | Loaded through | Needs `runtimeVersion`? |
| --- | --- | --- |
| `JAVA` — also Kotlin, Scala | `JAVA_TOOL_OPTIONS` | No |
| `PYTHON` | `PYTHONPATH` | No |
| `NODEJS` | `NODE_OPTIONS` | No |
| `DOTNET` | `CORECLR_*` / `DOTNET_*` | No |
| `PHP` | `PHP_INI_SCAN_DIR` | **Yes** |
| `RUBY` | `RUBYOPT` | **Yes** |

There's no auto-detection here, and unlike Lambda that isn't a stylistic choice. A Lambda function declares
its runtime as a property CDK can simply read. A container only declares an *image* — working out the
language would mean downloading and inspecting that image, which is network I/O a build can't do.
...
<ParamField path="runtimeVersion" type="string">
  ...
  and Ruby, ignored for everything else.
...
<Warning>
  **PHP and Ruby can only speak HTTP** — their community SDKs support nothing else.
...
The copy step pulls its files from an image — by default the one published by this package's release,
built for both Intel and ARM/Graviton.
...
<ParamField path="agentsImage" type="string">
  Point the copy step at your own mirror of the image:
...
A mirror has to be the same bundle — the environment variables name exact paths inside it. The bundle is
Linux and glibc only: a Windows task definition stops the build, and Alpine/musl images have no .NET
profiler available.
...
<Note>
  ...
  images. There's one copy step and it can't serve two different bundles.
</Note>
...
| `FARGATE_AGENTS_DIR`, `FARGATE_VOLUME_NAME`, `FARGATE_SIDECAR_CONTAINER_NAME` | The mount path, volume name, and copy-step container name. |
