...
```bash
npm install @odigos/aws-cdk@<PACKAGE_VERSION>       # Python: pip install odigos-aws-cdk==<PACKAGE_VERSION>
```
...
<Card title="Full CDK reference" icon="code" href="/cloud-connectors/aws/workloads/lambda/cdk-reference">
  Every option in depth — instrumenting a whole stack at once, the AWS Lambda Web Adapter, per-signal
  opt-in, Java handler interfaces, and the complete API.
</Card>
