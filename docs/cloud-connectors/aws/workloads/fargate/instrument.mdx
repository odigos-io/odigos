...
description: 'Self-managed ECS Fargate instrumentation for Java, Python, Node.js, .NET, PHP, and Ruby using hand-written CloudFormation (or AWS CDK via aws-cdk-lib/aws-ecs).'
...
<Warning>
  Unlike Lambda, there's no Odigos CDK construct for Fargate yet — support for it is
  planned. Until then, if you use CDK, write the same task-definition changes using
  plain `aws-cdk-lib/aws-ecs` — see [AWS CDK](#aws-cdk) below. Once a construct
  ships, it'll be documented alongside the Lambda one under
  [CDK](/cloud-connectors/aws/cdk/lambda).
</Warning>
...
There's no packaged Odigos construct for Fargate — you write the same four moves shown in the
CloudFormation tabs above yourself, using the plain ECS constructs from `aws-cdk-lib/aws-ecs`:

```typescript
taskDefinition.addVolume({ name: 'odigos-agent' });

const stager = taskDefinition.addContainer('odigos-agent', {
  image: ecs.ContainerImage.fromRegistry('registry.odigos.io/odigos-fargate-agents:<version>'),
  essential: false,
  command: ['sh', '-c', 'mkdir -p /var/odigos && cp -r /instrumentations/<lang> /var/odigos/<lang>'],
});
...
There's no `addEnvironment` on an ECS container after the fact — pass the same `Environment` block shown in
the CloudFormation tabs above as the `environment` prop when you call `addContainer` for your application
container.
