...
| `aws.fargate-task` | Workload | Yes | Yes (agent-staging sidecar in the task definition) |
| `aws.ecs-cluster` | Compute platform | Yes | Coming soon |
| `aws.fargate-cluster` | Compute platform | Yes | Coming soon |

<Info>
  `aws.ecs-cluster` and `aws.fargate-cluster` are discovered for coverage today;
  Odigos does not yet install a separate agent onto them. `aws.fargate-task` — an
  individual Fargate **service** — is the ECS resource Odigos instruments directly.
</Info>

aws.lambda and aws.fargate-task support two instrumentation approaches:
- Odigos-managed: The connector automatically applies and maintains the required changes.

- Self-managed: You maintain full control by provisioning through your own CloudFormation, CDK, or Terraform pipelines.

(For a deeper comparison, see [Odigos-managed vs. self-managed instrumentation](/cloud-connectors/overview#odigos-managed-vs-self-managed-instrumentation).)
For the self-managed path, artifact preloading, and per-language CDK/CloudFormation snippets, see the
[AWS Connector](/cloud-connectors/aws/overview) section.
  ...
  <Step title="Choose access level">
    ...
    <Tabs defaultTabIndex="1">
      <Tab title="Discovery only">
        ...
        | Lambda discovery | `lambda:ListFunctions`, `lambda:ListTags` |
        | ECS / Fargate discovery | `ecs:ListClusters`, `ecs:DescribeClusters`, `ecs:ListServices`, `ecs:DescribeServices`, `ecs:ListTasks`, `ecs:DescribeTasks`, `ecs:DescribeTaskDefinition` |

        <Info>
          `ecs:DescribeTaskDefinition` is a discovery permission, not only an
          instrumentation one — it's what lets Odigos see whether a self-managed
          Fargate service already carries your own instrumentation, so grant it
          even if you only ever plan to self-manage instrumentation.
        </Info>
      </Tab>
      <Tab title="Discovery + instrumentation">
        ...
        Includes all discovery actions, plus Odigos-managed instrumentation for Lambda and Fargate:
        ...
        | Manage Fargate task definitions | `ecs:RegisterTaskDefinition`, `ecs:UpdateService` |
        | Pass task/execution roles when registering a task definition | `iam:PassRole` (scope with condition `iam:PassedToService=ecs-tasks.amazonaws.com`) |

        Odigos-managed instrumentation also requires an [OTLP destination](/cloud-connectors/instrument-workloads#add-an-otlp-destination) after the connector is online. If you plan to self-manage instrumentation instead (your own CloudFormation/CDK/Terraform), you only need the discovery actions above — see [AWS Connector](/cloud-connectors/aws/overview).
      </Tab>
      <Tab title="Custom">
...
<CardGroup cols={2}>
  ...
  >
    Add an OTLP destination, create Sources for discovered Lambdas and Fargate services, and track instrumentation.
  </Card>
  <Card
    title="AWS Connector"
    icon="aws"
  ...
  >
    Self-managed instrumentation, artifact preloading, and CDK/CloudFormation for every language.
  </Card>
</CardGroup>
