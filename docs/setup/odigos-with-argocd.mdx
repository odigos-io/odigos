...
```bash
export ARGOCD_NS=<YOUR-ARGOCD-NAMESPACE>
export ODIGOS_NS=<YOUR-ODIGOS-NAMESPACE>
```
Replace `<YOUR-ARGOCD-NAMESPACE>` with the namespace where Argo CD is installed, and `<YOUR-ODIGOS-NAMESPACE>` with the desired namespace where you want Odigos to be installed.

## Step 4: Define the Argo CD Application Manifest
