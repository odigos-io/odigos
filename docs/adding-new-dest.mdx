...
<Steps>
  <Step title="Add Destination Type">
    Go to [`destination/dests.go`](https://github.com/odigos-io/odigos/tree/main/destination/dests.go) and add your new destination to the `DestinationType` enum.
    - Make sure the value is the same as the `type` property in `mydest.yaml`
  </Step>
  <Step title="Add Collector Config">
    Go to [`destinations/config`](https://github.com/odigos-io/odigos/tree/main/destinations/config) directory and create a new GO file.
    - Example: `mydest.go`

    <AccordionGroup>
      <Accordion icon="circle-info" title="Explain Config">
        - **DestType** - Returns the enum value of the destination added to [`destinations/dests.go`](https://github.com/odigos-io/odigos/tree/main/destinations/dests.go) in step 1.
        - **ModifyConfig** - Is called with the `dest` object which holds the data received from the UI and the `currentConfig` object. The `currentConfig` object contains the current configuration of the gateway collector. Modify this object to include the [OpenTelemetry configuration](https://opentelemetry.io/docs/collector/configuration/) needed by your destination. You can assume a basic configuration is already provided in the `currentConfig` object, for details see `getBasicConfig` method in [`destinations/config/root.go`](https://github.com/odigos-io/odigos/tree/main/destinations/config/root.go) file.
    ...
    func (m *MyDest) DestType() common.DestinationType {
      // DestinationType defined in destinations/dests.go
  ...
  <Step title="Add Available Config">
    Go to [`destinations/config/root.go`](https://github.com/odigos-io/odigos/tree/main/destinations/config/root.go) and add the new destination to the `availableConfigers` list.
    - Example: `&MyDest{}`

    ```go destinations/config/root.go
