...
In a [root-mode package install](/vmagent/setup/installation#root-and-non-root),
the agent manages `odigos-otelcol.service` as a separate systemd unit. A
**non-root** package install (the default) runs `odigos-otelcol` as a supervised
child of `odigos-vmagent` — the same model as this Docker deployment, where
...
```shell
sudo mkdir -p /var/lib/odigos-agent/config
```
...
<Tabs>
      ...
      services:
        odigos-vmagent:
          ...
          volumes:
            ...
            - /var/lib/odigos-agent/config:/var/lib/odigos-agent/config
  ...
  <Tab title="docker run">
    ...
    docker run -d --name odigos-vmagent \
      ...
      -v /var/lib/odigos-agent/config:/var/lib/odigos-agent/config \
...
|---|---|---|
| `ODIGOS_ONPREM_TOKEN` | Yes, unless `license.jwt` is pre-provisioned in the config directory | Written to `/var/lib/odigos-agent/config/license.jwt` at startup. |
...
The config tree is persisted on the host at `/var/lib/odigos-agent/config`, so the
...
<Note>
  Pre-provisioning `license.jwt` in `/var/lib/odigos-agent/config` is an alternative
...
| `/var/lib/docker` (ro) | Executable path resolution through overlay2, and container JSON logs for the [logs pipeline](/vmagent/setup/configuration/overview#log-collection). |
| `/var/odigos` (rw) | Instrumentation files must live on the host so host processes can load them. |
| `/var/lib/odigos-agent/config` (rw) | Agent configuration and identity, persisted across container upgrades. |
...
`/var/lib/odigos-agent/config` directory preserves the agent's identity and
...
```shell
sudo rm -rf /var/lib/odigos-agent
```
...
Config changes are written under `/var/lib/odigos-agent/config` on the host and persist
