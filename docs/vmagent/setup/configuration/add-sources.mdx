...
<Note>
  A **non-root** agent requires additional permissions to restart services, containers, and processes. See [Additional permissions](/vmagent/setup/permissions#additional-permissions).
</Note>
...
<Tabs>
  ...
  <Tab title="YAML">
    <Steps>
      <Step title="Navigate to the /var/lib/odigos-agent/config/sources.d folder">
    
        ```shell
        cd /var/lib/odigos-agent/config/sources.d
        ```
