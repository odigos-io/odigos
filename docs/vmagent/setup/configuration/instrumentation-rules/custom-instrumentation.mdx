  ...
  <Tab title="odictl">
    <Steps>
      ...
      <Step title="Configure the rule">
        ...
        2. For **Java**: set `className` (fully qualified class name) and `methodName` (method name).
        ...
        5. For **C++**: set `signature` (the function signature, for example `std::vector::push_back` or `SSL_write`).
        6. When finished, press `Esc`, then type `:wq` to save and exit.
        ...
        **Examples**
        ...
        <AccordionGroup>
          ...
          <Accordion title="C++">
            Instrument a function by its signature:
            ```yaml
            name: my-custom-instrumentation
            type: CustomInstrumentation
            config:
              cpp:
                - signature: "ecommerce::OrderProcessor::processOrder"
            ```
          </Accordion>
        </AccordionGroup>

        <Tip>The `java:`, `golang:`, and `cpp:` keys accept a list; add multiple entries to instrument several methods or functions in one rule. Use only one language per rule (do not combine `java:`, `golang:`, and `cpp:`). For Go, use either functions or receiver methods in a single rule, not both.</Tip>
  ...
  <Tab title="YAML">
    <Steps>
      ...
      <Step title="Add the custom instrumentation configuration">
        Add a rule with `CustomInstrumentation` and one of `java`, `golang`, or `cpp`. For Go, use either a **function** (package + functionName) or a **receiver method** (package + receiverName + receiverMethodName), not both.
