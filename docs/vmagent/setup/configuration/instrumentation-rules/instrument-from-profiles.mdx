...
<Steps>
  <Step title="Open the profiling view and select a function">
    ...
    In this example, the `odigos-demo-frontend` service is selected and the highlighted function is the Java method `sun.net.www.protocol.http.HttpURLConnection.getHeaderField`.

    <img
      src="../../../../images/vmagent/profiling/odictl_profiling_before_instrumenting.png"
      alt="odictl profiling view with the getHeaderField function selected, before instrumenting"
      style={{
        width: "700px",
        height: "auto",
        display: "block",
  ...
  <Step title="Instrument the function">
    Press <Badge color="blue">i</Badge>. The bottom-right panel updates to show the function is **Instrumented**. In this example, `getHeaderField` is now instrumented under the `odigos-profiling-odigos-demo-frontend` rule.

    <img
      src="../../../../images/vmagent/profiling/odictl_profiling_after_instrumenting.png"
      alt="odictl profiling view showing the getHeaderField function marked as Instrumented"
      style={{
        width: "700px",
        height: "auto",
        display: "block",
  ...
  <Step title="Verify the new span">
    Once the rule is applied, new traces include a span for the instrumented function. Open your trace backend and search for the function name (for example, `getHeaderField`) to confirm the span is being recorded.
  </Step>
</Steps>
