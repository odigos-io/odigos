...
| `systemd` | Streams logs via the journald receiver for the service |

<Note>
  A non-root agent can only read the systemd journal if the `odigos` user is in `adm` and/or `systemd-journal`. See [Linux Permissions](/vmagent/setup/permissions#systemd-journal-group-logs).
</Note>
