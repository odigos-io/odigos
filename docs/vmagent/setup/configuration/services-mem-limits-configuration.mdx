...
The Odigos VM Agent runs as two systemd services on your host **when the agent is in root mode**:
...
In the default **non-root** install, `odigos-otelcol` is a child of `odigos-vmagent` and they share one cgroup. Set memory limits on `odigos-vmagent` only (the packaged `nonroot.conf` already raises `MemoryMax` to 3G to cover both). Do not enable or edit `odigos-otelcol.service` in that mode.
...
<Note>
In non-root mode the collector shares the agent's cgroup, so the relevant defaults are those in `nonroot.conf`: `MemoryHigh=2500M`, `MemoryMax=3G`, `GOMEMLIMIT=750MiB`. Override them on `odigos-vmagent`, not on `odigos-otelcol.service`.
</Note>
