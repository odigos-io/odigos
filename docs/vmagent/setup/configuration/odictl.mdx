...
Enables **continuous CPU profiling** (eBPF, OTLP Profiles) on **odigos-otelcol**. Separate from VM Agent **pprof** (process debugging).

| Field | Description |
|-------|-------------|
| **profiling.enabled** | Turn on the `profiling` receiver and `profiles` export pipeline. Default `false`. |
...
Toggling profiling reloads collector config and syncs the PID registry. **Enabling profiling after the vm-agent process started with it off** requires a **vm-agent restart** for `GET_PROF_ATTR` on `/var/exchange/exchange.sock`.
