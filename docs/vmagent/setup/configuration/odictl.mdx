...
Apply the Odigos Pro token the support team issued. `odictl` validates the JWT before writing `/var/lib/odigos-agent/config/license.jwt`, then you restart the agent:

***Alternative***:

Create a file called `/var/lib/odigos-agent/config/license.jwt` with the JWT token and restart the agent.
...
See [Installation](/vmagent/setup/installation#apply-the-license).
...
Controls how the agent instruments discovered processes. The Java instrumentation
subsection targets Java-specific behavior.
...
| Field | Description |
|-------|-------------|
| **profiling.enabled** | Turns on the `profiling` receiver and the `profiles` pipeline whenever a destination is configured with the `PROFILES` signal. Default `false`. |
...
Toggling this field reloads the collector and the PID → `service.name` registry within seconds — no vm-agent restart is required.

</Accordion>

<Accordion title="Pprof" icon="bug" defaultOpen="false">

Controls the **Go `net/http/pprof`** debug endpoint for the **vm-agent process only**. This is for debugging the agent itself and has nothing to do with application continuous CPU profiling (see **Profiling** above for that).

| Field | Description |
|-------|-------------|
| **Enabled** | Expose pprof on `network.vmagent.pprof_port` (default `7771`). Enabled by default. |
