...
Enables **continuous CPU profiling** (eBPF, OTLP Profiles) on **odigos-otelcol**. This is separate from Go **pprof** (see **Pprof** below).

| Field | Description |
|-------|-------------|
| **profiling.enabled** | Turns on the `profiling` receiver and `profiles` pipeline when a destination includes **PROFILES**. Default `false`. |
...
When you toggle profiling, the agent reloads collector config and syncs the PID→`service.name` registry. **Enabling profiling after the vm-agent process started** still requires a **vm-agent restart** so the unix socket accepts `GET_PROF_ATTR`; the collector itself is reloaded, not fully restarted.

</Accordion>

<Accordion title="Pprof" icon="bug" defaultOpen="false">

Controls the **Go `net/http/pprof`** debug endpoint for the **vm-agent process only** (not application CPU profiling).
