...
Enable the signals you want to export on each destination (`TRACES`, `LOGS`, `PROFILES`). The agent turns on collection for those signals automatically—for example, configuring `LOGS` starts [log collection](/vmagent/setup/configuration/overview#log-collection) for instrumented sources.
...
<Tabs>
  ...
  <Steps>
    ...
    <Step title="Configure destination">
      ...
      <Frame
        caption="Example of an [OTLP http](/enterprise/backends-overview) destination"
        style={{
          width: "450px",
          margin: "0 auto"
        }}
      ...
      Fill in the required fields, then press `Ctrl+S` to save your destination.
  ...
  <Tab title="YAML">
    <Steps>
      <Step title="Navigate to the /var/lib/odigos-agent/config/destinations.d folder">
    
        ```shell
        cd /var/lib/odigos-agent/config/destinations.d
        ```
      ...
      ```
      Mar 11 20:50:18 ip-10-0-1-51 odigos-vmagent[611]: time=2026-03-11T20:50:18.401Z level=INFO source=/go/src/github.com/keyval/odigos-vmagent/pkg/components/controller/destination/controller.go:177 msg="Destination signals configured" destination=otlphttp signals=[TRACES LOGS]
      ```
