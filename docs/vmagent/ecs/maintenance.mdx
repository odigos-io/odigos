...
description: "Roll out a new Odigos ECS Agent version, switch between deployment modes, and remove Odigos from an ECS cluster."
---
...
No reinstall — the agents speak the same protocol to the proxy that they speak
to Central.

<Steps>
  <Step title="Deploy the proxy">
    Follow [Install: Agent + Proxy](/vmagent/ecs/installation-with-proxy) steps
    3–5 (stable address, proxy task definition, proxy service).
  </Step>
  <Step title="Re-point the agents">
    In the agent task definition, change `ODIGOS_DEFAULT_CONFIG`'s endpoint to the
    proxy's `host:4321` **and** add `ODIGOS_ECS_BEHIND_PROXY=true`. Register the
    revision.
  </Step>
...
The cluster then appears in Central as one platform named after the cluster,
replacing the per-node platforms.

<Note>
  Going the other way works the same: point the endpoint back at Central, remove
  `ODIGOS_ECS_BEHIND_PROXY` (or set it to `false`), roll the daemon, then delete
  the proxy service.
</Note>
