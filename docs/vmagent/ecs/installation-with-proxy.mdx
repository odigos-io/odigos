...
<Steps>
  <Step title="Choose the proxy's stable address">Pinned instance, or an internal NLB.</Step>
  <Step title="Deploy the proxy">One task per cluster, listening on TCP 4321.</Step>
  <Step title="Deploy the agent daemon">Pointed at the proxy, with `ODIGOS_ECS_BEHIND_PROXY=true`.</Step>
</Steps>
...
<Warning>
  **`ODIGOS_ECS_BEHIND_PROXY=true` matters.** The agent cannot tell a proxy
  ...
  Leave it `false` here and every agent registers under the cluster name instead,
