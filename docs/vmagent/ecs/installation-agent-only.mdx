...
<Accordion title="odigos-ecs-agent.json">
  ...
  Every setting that looks unusual is required for a specific capability — see
  [required permissions](/vmagent/ecs/configuration#required-permissions) before
...
<Note>
  In this mode `odictl` targets **one node's** agent — there is no cluster-wide
