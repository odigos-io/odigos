...
description: "The Odigos VM Agent instruments Linux processes and systemd services using eBPF. Because instrumentation runs in the kernel, impact on applications is negligible. Traces, logs, and profiles are exported to any of the supported destinations of your choice."
---
...
- Collect application logs from instrumented sources (process, Docker, systemd) and export them when a destination accepts the `LOGS` signal
...
The VM Agent can be deployed in three ways. All of them instrument workloads with
the same eBPF agent and are configured the same way (via Odigos Central or
`odictl`):
...
- **Amazon ECS daemon service** — run the agent as an ECS daemon so every EC2
container instance instruments the ECS tasks on it, with no task restarts. See
[Odigos on Amazon ECS](/vmagent/ecs/overview).
