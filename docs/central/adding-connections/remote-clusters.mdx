...
Do **not** set `ui.centralBackendURL` on a Central-managed cluster. That value is only for the per-cluster Odigos UI. Setting both writes duplicate `centralBackendURL` keys into `odigos-configuration`. See [Architecture](/central/architecture#backend-urls-helm-values).
