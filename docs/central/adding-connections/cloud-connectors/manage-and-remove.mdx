...
<AccordionGroup>
  <Accordion title="Common causes of Degraded or Error">
    ...
    **Error** usually means:
    ...
    Fix credentials or IAM, confirm [Cloud Connectors are enabled](/central/adding-connections/cloud-connectors/enable), and check `connector-runtime` / connector pods in the Central namespace.
  </Accordion>
</AccordionGroup>
...
<Tabs>
  <Tab title="retain (default)">
    ...
    Any Odigos instrumentation already applied in the cloud account stays in place (for example Lambda layers or Cloud Run agent injection). To remove that instrumentation, [uninstrument Sources](/central/adding-connections/cloud-connectors/instrument-workloads#uninstrument-workloads) while the connector is still online, or change those resources outside Odigos after delete.
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="cloud"
    href="/central/adding-connections/cloud-connectors/overview"
  >
