...
| `aws.ecs-cluster` | Compute platform | Yes | Agent install coming soon |
| `aws.fargate-cluster` | Compute platform | Yes | Agent install coming soon |

## Add the connector

<Steps>
  <Step title="Open Add connector and choose AWS">
    In the Central UI, open Connections / Connectors and start **Add connector**. Choose **Amazon Web Services**.
  </Step>
  ...
  <Step title="Choose access level">
    ...
    <Tabs defaultTabIndex="1">
      ...
      <Tab title="Discovery + instrumentation">
        ...
        Lambda instrumentation also requires an [OTLP destination](/central/adding-connections/cloud-connectors/instrument-workloads#add-an-otlp-destination) after the connector is online.
      </Tab>
      <Tab title="Custom">
  ...
  <Step title="Setup credentials">
    ...
    <Tabs>
      <Tab title="Assume Role">
        ...
        The UI guides you through **creating the IAM role** using the workflow you prefer. Choose one.  Each path ends with the same role (trust policy + access-level permissions) and a Role ARN to paste back into Odigos.
  ...
  <Step title="Verify and create">
    ...
    | --- | --- |
    | Authentication | Credentials resolve and `sts:GetCallerIdentity` succeeds |
    | Account match | Caller identity belongs to the configured account ID |
    | Region access | `ec2:DescribeRegions` succeeds when discovery is enabled |
    | Lambda discovery permissions | `lambda:ListFunctions` when Lambda discovery is enabled |
    | ECS discovery permissions | `ecs:ListClusters` when any ECS/Fargate discovery type is enabled |
