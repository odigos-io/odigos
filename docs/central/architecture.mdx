...
| `ui.centralBackendURL` | `odigos` (remote cluster) | Per-cluster Odigos UI | **Do not set** when you manage the cluster from Central UI only. |
...
If you set both `centralProxy.centralBackendURL` and `ui.centralBackendURL` on a remote cluster, the `odigos-configuration` ConfigMap writes **two** `centralBackendURL` keys inside `config.yaml` (YAML keeps the last one). That ConfigMap has a `config.yaml` file, not a top-level key named `centralBackendURL`. Do not set `ui.centralBackendURL` for Central-managed clusters.
