...
<Warning>
  When configuring the pipeline, you must use **only one of the two methods** — either `profiles`, or directly editing the `odigos-config` ConfigMap.
...
Sizing Profiles `size_xs`, `size_s`, `size_m`, `size_l` are pre-defined configurations designed to simplify pipeline configurations. Each profile specifies the following parameters:
...
|----------|-----------------------|----------------------|-----------------|---------------|----------------------|-------------------|
| `size_xs` | **1**                | **4**                | **113m**        | **225m**      | **225Mi**            | **225Mi**         |
...
<Note>
  These sizing profiles also affect the resource requirements of core Odigos components (autoscaler, instrumentor, scheduler, and UI).
  For detailed resource specifications of all components, see [Available Profiles](/profiles/available-profiles#core-component-resource-requirements).
</Note>
