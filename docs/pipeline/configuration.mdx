...
- **Cluster Gateway Collector** - a collector that runs as a k8s Deployment. It receives the OpenTelemetry data from the Node Collectors, processes it, and exports it to the next component in the pipeline.
...
Profiles are sets of pre-defined rules, specifically sizing profiles (`size_s`, `size_m`, `size_l`), to simplify the configuration of the pipeline. Each profile configures the following parameters for the **Cluster Gateway Collector**:
