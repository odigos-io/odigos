...
- The action will be positioned after all other sampling actions, serving as a fallback to handle any traces not selected by the preceding actions. It can be used if you do not want to see all traces that were not dropped by other sampling actions.