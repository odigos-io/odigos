...
description: "Sample traces based on the duration of a trace."
...
The "Latency Sampler" Odigos Action is an [Endpoint Action](/pipeline/actions/introduction#scope-categories) that samples traces based on their duration for a specific service and endpoint (HTTP route) filter.
