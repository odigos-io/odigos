...
This action will reduce the collected traces by dropping traces for kubernetes health checks. 
Helps to reduce cost and noise by filtering out "uninteresting" traces with low value and high volume.
...
- Health checks are sampled at the agent level (head sampling) before traces are pushed into the collector pipeline, regardless of any other sampling configuration.
- Only available in supported agents.
