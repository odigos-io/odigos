...
Sampling Actions allow you to configure various types of sampling methods before exporting them to your Odigos Destinations.
...
- If two rules are configured: an Endpoint action for `Service: Frontend, Endpoint: /buy` with a latency threshold of 1 second and a fallback of 50%, and a Global ErrorSampler with a fallback of 20%:
  ...
  1. Traces from `Frontend /buy` below 1 second latency will fall under the Endpoint action's fallback and retain 50% of these traces.
  2. Traces from `Frontend /buy` above 1 second latency will always be retained.
  3. Error traces will always be sampled due to the Global ErrorSampler.
