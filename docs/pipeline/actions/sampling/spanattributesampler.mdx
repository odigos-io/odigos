...
description: "A service-scoped action that samples traces based on the presence, value, or structure of specific span attributes—including support for JSON evaluation."
...
<Warning>
  Before enabling **latency sampler**, please note the following: - Supports
  only traces. - Multiple `endpoint_filters` can be configured within the same
  action. - Adding this action causes a 30-second delay in sending the data. -
...
### Cost Control via Precision Sampling

Apply a fallback sampling ratio for non-matching traces to reduce data volume while still capturing a representative subset.
...
### 1. Match: Error in Checkout Service
