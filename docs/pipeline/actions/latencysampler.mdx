...
Configuring minimum_latency_threshold for a specified service and endpoint will sample any request with a duration exceeding this threshold for that particular service and endpoint combination.
...
- `service` (required): The rule applies to a specific service name. Only traces from this service will be considered.

- `endpoint` (required): The specific HTTP route prefix to match for sampling. Only traces with routes beginning with this prefix will be considered. For instance, configuring /buy will also match /buy/product.
