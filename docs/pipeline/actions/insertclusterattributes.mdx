...
sidebarTitle: "Insert Cluster Attributes"
---

The "Insert Cluster Attributes" Oridog Action can be used to add resource attributes 
...
```yaml
apiVersion: action.odigos.io/v1alpha1
...
The full list of options available for the "InsertClusterAttributes" action are:
...
- If `attributeStringValue` is unset or `null`, the attribute will be added with a `null` value.
