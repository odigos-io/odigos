---
title: "Pii Masking"
sidebarTitle: "Pii Masking"
---

The "PII Masking" Odigos Action can be used to mask PII data from span attribute values.

### Use Cases

#### Security

- By default, OpenTelemetry should not record PII (Personally Identifiable Information) or sensitive data such as passwords, api tokens, etc. However, if the data is accidentally added to the telemetry signals, or if a manual instrumentation recorded it into an attribute, this action can be used to mask such data.

#### Legal and Compliance

- Ensure compliance with legal and privacy.
    - EU General Data Protection Regulation (GDPR) prohibits txhe transfer of any personal data like birthdates, addresses, or ip addresses across borders without explicit consent from the data subject.
    - Health Insurance Portability and Accountability Act (HIPAA) requires that all health data be protected from unauthorized access.
    - Payment Card Industry (PCI) Data Security Standards prohibit logging certain things or storing them unencrypted.


### Mechanism

This action receives a list of all the PII Categories to mask from the traces.
The category of the PII data to be masked is associated with a regex expression that will be applied to all span attribute, replacing any matches with `****`.



### Basic Example

The basic example below shows how to configure the Pii Masking action to mask Visa and Master cards from span attribute values.

```yaml

Create a file named `piimasking.yaml` with the following content:

```yaml
apiVersion: actions.odigos.io/v1alpha1
kind: PiiMasking
metadata:
  name: piimasking
  namespace: odigos-system
spec:
  actionName: "PII Masking"
  piiCategoryConfigurations:
    - mask: true
      category: CREDIT_CARD
  signals:
    - TRACES
```

Apply the action to the cluster:

```bash
kubectl apply -f piimasking.yaml
```

### Full Action Options

The full list of options available for the "PiiMasking" action are:

- `piiCategoryConfigurations` (required): An array of dictionaries where each dictionary has two keys: `mask` and `category`. `category` is the PII Category you want to mask and `mask` is a boolean value that indicates if to mask or not.

- `signals` (required): An array with the signals that the processor will act on (`TRACES`).

- `actionName` (optional): Allows you to attach a meaningful name to the action for convenience. Odigos does not use or assume any meaning from this field.

- `notes` (optional): A free-form text field that allows you to attach notes to the action for convenience. Odigos does not use or assume any meaning from this field.

- `disabled` (optional): A boolean field that allows you to disable the action. When set to `true`, the action will not be executed. The default value is `false`.

### Notes

- This action will mask span attribute values by replacing only the problematic portions with `****`, leaving the rest of the value unchanged.
- Currently, supports only trace signals.
- All span attribute values in all spans will be checked and masked.
