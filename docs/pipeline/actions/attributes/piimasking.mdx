...
- Ensure compliance with legal and privacy.
    - EU General Data Protection Regulation (GDPR) prohibits txhe transfer of any personal data like birthdates, addresses, or ip addresses across borders without explicit consent from the data subject.
    - Health Insurance Portability and Accountability Act (HIPAA) requires that all health data be protected from unauthorized access.
    - Payment Card Industry (PCI) Data Security Standards prohibit logging certain things or storing them unencrypted.


### Mechanism

This action processes a list of PII categories to be masked from the traces. Each PII category is linked to a regex expression that will be applied to all span attribute values. Any portion of the span attribute values that match the regex expression will be replaced with `****`, while the rest of the attribute value will remain unchanged.


### Basic Example

The basic example below shows how to configure the Pii Masking action to mask Visa and Master cards from span attribute values.
...
- `piiCategoryConfigurations` (required): An array of dictionaries where each dictionary has two keys: `mask` and `category`. `category` is the PII Category you want to mask and `mask` is a boolean value that indicates if to mask or not.
...
- The regex expressions used for masking are predefined for each PII category and cannot be configured.
...
- The regex expressions used for masking are predefined for each PII category and cannot be configured.
- Currently, supports only trace signals.
- All span attribute values in all spans will be checked and masked.
