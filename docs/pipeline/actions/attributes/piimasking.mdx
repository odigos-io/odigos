...
- `piiCategories` (required): An array of strings representing the PII Category you want to mask. 
   The available PII categories are:
    ...
    The `CUSTOM` category allows you to define your own regex expression to mask a specific PII category. The `ALL` category masks all the PII categories. The `NONE` category does not mask any PII category.
