---
title: "Processor CRD"
sidebarTitle: "Processor CRD"
---

You can define OpenTelemetry Collector Processors in Odigos using the kubernetes `Processor` CRD. This option is for advanced users who are familiar with the OpenTelemetry Collector and it's configuration.
...
Adding a processor to your Odigos pipeline can be done by creating a `processor` resource in your k8s cluster, for example:
...
- `data` (optional): A field to pass configuration to the processor. The structure of this field is specific to each processor, and you can find the configuration options for each processor in the [OpenTelemetry Collector Contrib codebase](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/processor).

- `signals` (required): An array with the signals that the processor will act on (`TRACES`, `METRICS`, `LOGS`).

- `collectorRoles` (required): An array with the collector roles that the processor will act on (`GATEWAY`, `DATA_COLLECTION`). It is generally enough to apply the processor in just one collector, depending on the use case.
