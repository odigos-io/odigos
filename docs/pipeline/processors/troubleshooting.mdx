...
How to troubleshoot common issues with the processors:
...
If there is an issue with your processor configuration, the collector will log an error and will not start:
...
The `odigos-autoscaler` controller is the component that watch for changes in the processor CRD and generates the collector confgiuration.
