...
- **Kubernetes Operators** - process changes in the cluster resources and deploy / configure the necessary components.
- **DaemonSets** - performs any node-level tasks, like when interacting with a process running on the node OS, accessing Kubelet, and reading logs from the node filesystem.
...
- Instrumentation
  - inject OpenTelemetry agents to record / collect the instrumentation data from Odigos sources (instrumented applications),
  - record and report any instrumentation-relevant runtime information that is available in the process environment,
  - export the collected telemetry to the pipeline.

- Pipeline
  - offload any common or heavy task from the instrumented application runtime,
  - apply any Odigos actions on the telemetry data to modify it according to user's needs,
...
**Role**: The auto scaler component is responsible for managing the OpenTelemetry Collectors which implements the OpenTelemetry pipeline.
...
- Serve as web server for the Odigos UI:
  - serves static files to the frontend,
  - provides a GraphQL API for the frontend, which in turn interacts with Kubernetes API to manage the Odigos installation.
